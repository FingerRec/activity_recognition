...
class BNInception(nn.Module):
    ...
    def __init__(self, num_classes=101, channel=20):
        super(BNInception, self).__init__()
        inplace = True
        self.conv1_7x7_s2_custom = nn.Conv2d(channel, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3))
        ...
        self.last_linear_custom = nn.Linear (1024, num_classes)

    def features(self, input):
        conv1_7x7_s2_out = self.conv1_7x7_s2_custom(input)
...
def bninception_101(num_classes=101, pretrained='imagenet', channel = 20):
    r"""BNInception model architecture from <https://arxiv.org/pdf/1502.03167.pdf>`_ paper.
    """
    model = BNInception(num_classes=num_classes, channel=channel)
    if pretrained is not None:
        settings = pretrained_settings['bninception'][pretrained]
        assert num_classes == settings['num_classes'], \
            "num_classes should be {}, but is {}".format(settings['num_classes'], num_classes)
...
def weight_transform(model_dict, pretrain_dict, channel):
    ...
    w3 = pretrain_dict['conv1_7x7_s2.weight']
    print type(w3)
    if channel == 3:
        wt = w3
    else:
    ...
    weight_dict['conv1_7x7_s2_custom.weight'] = wt
