...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv2(out)
        out = self.bn2(out) #标准化
        out = self.relu(out)

        out = self.conv3(out)
        out = self.bn3(out) #标准化
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, nb_classes=101, channel=20):
        ...
        #每一层中都使用Batch Normalization.
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        #最大池化层
        #接下来为4个全局卷积层，区别仅仅在于个数不同
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes)) #增加块的数目

        return nn.Sequential(*layers)#时许容器，Modules 会以他们传入的顺序被添加到容器中
...
#resnet101为preTrained模型，【3，4，23，3]为每个卷积层的个数
...
#权重转换，下载得到的image-net上pretrain的模型权重更新给model_dict
