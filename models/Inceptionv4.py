...
class InceptionV4(nn.Module):
    ...
    def __init__(self, num_classes=1001, channel = 3):
        ...
        self.std = None
        # Modules 更改为20通道
        self.conv1_custom = BasicConv2d(channel, 32, kernel_size=3, stride=2)
...
def inceptionv4_101(num_classes=101, pretrained='imagenet', channel = 3):
    if pretrained:
        settings = pretrained_settings['inceptionv4'][pretrained]
        assert num_classes == settings['num_classes'], \
            "num_classes should be {}, but is {}".format(settings['num_classes'], num_classes)
        ...
        # both 'imagenet'&'imagenet+background' are loaded from same parameters
        model = InceptionV4(num_classes=101, channel = channel)
        #从各层引入权重初始化，改为
      #  model.load_state_dict(model_zoo.load_url(settings['url']))
        #注意这里没有初始化最后一层的权重
        pretrain_dict = model_zoo.load_url((settings['url']))                  # modify pretrain code
        model_dict = model.state_dict()
        model_dict = weight_transform(model_dict, pretrain_dict, channel)
        model.load_state_dict(model_dict) #初始化权重


#        if pretrained == 'imagenet':
#            new_last_linear = nn.Linear(1536, 101)
#            new_last_linear.weight.data = model.last_linear.weight.data[1:]
#            new_last_linear.bias.data = model.last_linear.bias.data[1:]
#            model.last_linear = new_last_linear
...
def cross_modality_pretrain(conv1_weight, channel):
    ...
    for i in range(3):
        S += conv1_weight[:,i,:,:]
    avg = S/3.
    new_conv1_weight = torch.FloatTensor(32, channel, 3, 3)
    #print type(avg),type(new_conv1_weight)
    for i in range(channel):
        new_conv1_weight[:,i,:,:] = avg
    return new_conv1_weight
#权重转换，下载得到的image-net上pretrain的模型权重更新给model_dict,需要注意维度的问题
#ResNet 为RGB三通道，在temporal时候需要改变为20通道
def weight_transform(model_dict, pretrain_dict, channel):
    weight_dict  = {k:v for k, v in pretrain_dict.items() if k in model_dict}
    w3 = pretrain_dict['features.0.conv.weight']
    #print type(w3)
    if channel == 3:
        wt = w3
    else:
    ...
    weight_dict['features.0.conv.weight'] = wt
