...
"""
     ...
     # @File    : resnet_101_tsn_temp_motion_dataloader.py
     # @Software: PyCharm
...
from utils.utils import read_flow
...
class motion_dataset(Dataset):
    ...
    def stackopf10pervideo(self):
        ...
        flow = torch.FloatTensor(2 * self.in_channel, self.img_rows, self.img_cols)
        if self.video == 'PushUps_g16_c04': return flow
        i = int(self.clips_idx)
        with open(opt.flow_var_rank, 'r') as f:
            for line in f:
                line = line.strip()
                each_row = line.split(' ')
                if each_row[0] == name:
